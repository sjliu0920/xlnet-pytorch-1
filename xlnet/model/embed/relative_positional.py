...
class RelativePositionalEmbedding(nn.Module):
    ...
    def forward(self, qlen, klen, batch_size: int = None):
        ...
        pos_embed = self.get_positional_embed(
    ...
    def get_positional_embed(
        self, is_bidirectional, inv_freq, begin, end, clamp_len, batch_size
    ):
        if is_bidirectional:
            return self.get_bi_directional_positional_embed(
                inv_freq, begin, end, clamp_len, batch_size
            )
        return self.get_default_positional_embed(
            inv_freq, begin, end, clamp_len, batch_size
        )

    def get_bi_directional_positional_embed(
        self, inv_freq, begin, end, clamp_len, batch_size
    ):
        ...
        fwd_pos_emb = self.positional_embed.forward(
            fwd_pos_seq, inv_freq, embed_batch_size
        )
        bwd_pos_emb = self.positional_embed.forward(
            bwd_pos_seq, inv_freq, embed_batch_size
        )
